...
const { EntryNotFound, AuthorizationError } = require('../designers/designers');
...
module.exports.deletecardsid = (req, res, next) => {
  cards.findById(req.params.id)
    .then((card) => {
      ...
      if (card == null) return next(new EntryNotFound('Данная запись пустая'));
      return next(new AuthorizationError('Данная запись небыла удалена, так как не вы её создатель'));
    })
    .catch(() => next(new EntryNotFound('Данная запись не была найдена')));
};
